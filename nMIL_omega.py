...
class nMIL_omega:
    ...
    def grad_func(X, Y, orgX, w, beta, gamma, m0, p0):
        ...
        for i, superbag in enumerate(X):
            ...
            for j, bag in enumerate(superbag):
                bag_dotp = np.dot(bag, w[j, :][:, np.newaxis])
    ...
    def SGD(self):
        ...
        for expr in range(n_sgd):
            w = np.random.rand(self.bagsize, x_dimension)
            ...
            for idx, testx in enumerate(test_X):
                ...
                days_list = [np.mean(scipy_sig(np.dot(bag, w[j, :][:, np.newaxis])))
...
def main(args):
    ...
    start  = time.time()
    model = nMIL_omega(day, beta=args.beta, gamma=args.gamma, m0=args.m0, p0=args.p0)
    print "Learning for Bag Size: %d" % day
    model.read_data(histIndex=historyMap, docIndex=docMap, feature=args.feature)

    model, perf1, perf2, gsrHistoryProbs, test_Y, pred_probs = model.SGD()
    w1 = open(resultf, 'a')
    w1.write('\t'.join([str(score) for score in perf1]) + '\n')
    w1.close()
    outf = "../result/{}_{}_lt-{}_hd-{}_.json".format(args.outfile, args.country, args.leadtime, day)
    w2 = open(outf, 'wb')
    w2.write(json.dumps(gsrHistoryProbs))
    w2.close()
    print "Running nMIL omega model for forecasting on country %s, leadtime %d" % (args.country, args.leadtime)
